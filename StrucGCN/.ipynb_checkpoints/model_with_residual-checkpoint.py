...
class StrucGNN(nn.Module):
    def __init__(self, args, neigh_data, struc_data, merged_data):
        ...
        self.conv0 = torch.nn.Linear(self.num_features, args.h_dim)
        ...
        # self.conv1 = Encoder(args, args.h_dim, args.h_dim, self.neigh_data, self.struc_data, self.merged_data)
        # self.conv2 = Encoder(args, args.h_dim, args.h_dim, self.neigh_data, self.struc_data, self.merged_data)

    def forward(self):
        # h0 = F.relu(self.conv0(self.neigh_data.x))
        # h0 = F.dropout(h0, p=self.dropout_rate, training=self.training)
        ...
        h0 = self.conv0(self.neigh_data.x)
        h1 = self.conv1(self.neigh_data.x)
        h2 = self.conv2(h1)
        h2 = h2 + h0
        h = self.weight.mm(h2.t()).t()
