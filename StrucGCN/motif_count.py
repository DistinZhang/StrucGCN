...
class MotifCounterMachine(object):
    ...
    def create_edge_subsets(self):
        ...
        unique_subsets = dict()
        for i in range(3, self.args.motif_graphlet_size+1):
    ...
    def enumerate_graphs(self):
        ...
        self.interesting_graphs = {i: [] for i in range(2, self.args.motif_graphlet_size+1)}
        for graph in graphs:
            if graph.number_of_nodes() > 1 and  graph.number_of_nodes() < self.args.motif_graphlet_size+1:
    ...
    def create_tabular_motifs(self):
        ...
        for index in range(self.unique_motif_count): # 0-15
            ...
            if sum(features) > 0:
                ...
                features = pd.qcut(features, self.args.motif_quantiles, duplicates="drop", labels=False)
                features = dict(zip(ids,features))
                for node in self.graph.nodes():
                    self.binned_features[node].append(str(int(index*self.args.motif_quantiles + features[node]))) #node 作为index取features的值
    ...
    def factorize_string_matrix(self):
        ...
        model = NMF(n_components=self.args.motif_factors, init="random", random_state=self.args.seed, alpha=self.args.motif_beta)
        factors = model.fit_transform(features)
        kmeans = KMeans(n_clusters=self.args.motif_clusters, random_state=self.args.seed).fit(factors)
